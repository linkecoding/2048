...
function init(){
	for(var i=0; i<4; i++)
	{
		for(var j=0; j<4; j++)
		{
			var gridCell = $("#grid-cell-"+i+"-"+j);				//取得每一个格子
	...
	for(var i=0; i<4; i++)
	{
		for(var j=0; j<4; j++)
		{
			...
			else
			{
				...
				theNumberCell.css('background-color',getNumberBackgroundColor(board[i][j]));				//不同的数字不同的背景色
...
{
	if(nospace(board))					//有空位置
		return false;
	// 随机一个位置
	var randx = parseInt(Math.floor(Math.random()*4));				//0-4之间整数
...
document.addEventListener("touchmove",function(event){
	event.preventDefault();					//处理安卓4.0的BUG
});
